    ...
    public override bool Execute()
    {
        ...
        for (int i = 0; i < range; i++)
        {
            if (n.GetLinkInDirection(direction) != null)
            {
				...
				if (n.currentObject != null && n.currentObject.GetComponent<DestroyableProp>() != null) {n.currentObject.GetComponent<Health>().Damage(owner,1); }
                foreach (LinkPositions d in LinkPositions.GetValues(typeof(LinkPositions)))
                {
                    if (n.GetLinkInDirection(d) != null)
                    {
                        ...
                        if (c != null && c.GetComponent<DestroyableProp>() != null) { c.GetComponent<Health>().Damage(owner,1); }
                    }
                }
            }
        }
